...
//        if ($_SESSION['cur_user']) {
            ...
            $data = D('Message')->where("invalid_id=0 AND type='plain' AND status=0 AND id=%d", $id)->find();
...
//        $content = $_REQUEST["content"];
    ...
    {
        if ($id < 1) return -1;
        $prev = D('Message')->where("invalid_id=0 AND type='plain' AND status=0 AND id<" . $id)
            ->order('id desc')->find();
        if ($prev) {
            return $prev['id'];
        } else {
    ...
    {
        $next = D('Message')->where("invalid_id=0 AND type='plain' AND status=0 AND id>" . $id)
            ->order('id asc')->find();
        if ($next) {
            return $next['id'];
        } else {
