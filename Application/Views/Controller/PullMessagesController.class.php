...
//        http://localhost/superkzy/index.php/Views/PullMessages/allByTime?date1=1471041647&date2=1471128047
    ...
    {
        $where['invalid_id'] = 0;
        $where['type'] = array("IN",array('plain','wx_mp','group')); // 拉取朋友圈和公众号的消息，此处不拉group
        $where['status'] = 102;
        $where['category']=$category;
        $where['update_time'] = array("BETWEEN", array($date1, $date2));
        $results = D('message')->distinct(true)->field("content")->where($where)->order('content desc,update_time desc')->select();
...
//        echo "start ";
        ...
        if ($relation) {
            ...
            $where1['invalid_id'] = 0;
            $where1['type'] = array("IN",array('plain','wx_mp')); // 此处不能拉group
            $where1['id'] = $msg_id;
